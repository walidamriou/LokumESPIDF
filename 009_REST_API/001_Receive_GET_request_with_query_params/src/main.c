...
static esp_err_t hello_get_handler(httpd_req_t *req)
{
    ...
    if (buf_len > 1) {
        ...
        if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
            ...
            if (httpd_query_key_value(buf, "valure3", param, sizeof(param)) == ESP_OK) {
    ...
    sprintf(bb, "%d",b); 
    sprintf(cc, "%d",c); 
    httpd_resp_set_hdr(req, "you have sent Valure1", aa); // I  think the problem with a because
    httpd_resp_set_hdr(req, "you have sent Valure2", bb); // it's with type int and the function want char :)
